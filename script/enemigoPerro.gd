...
# ─── AnimationPlayer está como hijo directo de enemigo según la escena ───
@onready var anim: AnimationPlayer = $Sketchfab_Scene/AnimationPlayer

enum Estado { RONDANDO, ESPERANDO, PERSIGUIENDO, ATACANDO }
var estado: Estado = Estado.ESPERANDO

# ── Nombres exactos de las animaciones del modelo ──
const ANIM_IDLE    = "Model|Idle"
const ANIM_WALK    = "Model|Walk"
const ANIM_RUN     = "Model|Run"
const ANIM_ATTACK  = "Model|Attack"   # cambia a "Model|Attack2" si prefieres
const ANIM_HIT     = "Model|Hit"

func _reproducir_anim(nombre: String):
	if anim == null:
		print("ERROR: anim es null, no se encontró el AnimationPlayer")
		return
	if anim.current_animation == nombre:
		return
	print("Intentando reproducir: ", nombre)
	print("Animaciones disponibles: ", anim.get_animation_list())
	if anim.has_animation(nombre):
		anim.play(nombre)
		print("✓ Reproduciendo: ", nombre)
	else:
		print("✗ Animación NO encontrada: ", nombre)
...
func _actualizar_animacion():
	match estado:
		Estado.ESPERANDO:
			_reproducir_anim(ANIM_IDLE)
		Estado.RONDANDO:
			_reproducir_anim(ANIM_WALK)
		Estado.PERSIGUIENDO:
			_reproducir_anim(ANIM_RUN)
		Estado.ATACANDO:
			_reproducir_anim(ANIM_ATTACK)
...
func _physics_process(delta):
	...
	_actualizar_estado()
	_actualizar_animacion()   # ← se llama cada frame después de saber el estado
...
func _actualizar_estado():
	...
	# No interrumpas el ataque desde aquí
...
func _rondar(delta):
	...
	if tiempo_intentando_llegar >= tiempo_maximo_destino:
		...
		var lado = 1 if randi() % 2 == 0 else -1
		var direccion_nueva = direccion_actual.rotated(
			Vector3.UP, deg_to_rad(120 * lado)
		).normalized()
...
func _atacar_jugador():
	if jugador == null or _jugador_esta_escondido() or not is_inside_tree():
		return
	...
	# Espera a que termine la animación de ataque antes de seguir
	await get_tree().create_timer(tiempo_detencion_ataque).timeout

	if not is_inside_tree():
		return

	# Mira al jugador al terminar el golpe
	if jugador != null and not _jugador_esta_escondido():
		...
		if direccion.length() > 0.1:
			look_at(global_position + direccion.normalized(), Vector3.UP)

	estado = Estado.PERSIGUIENDO

	await get_tree().create_timer(delay_ataque).timeout
